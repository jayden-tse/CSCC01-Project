...
const { WRITE_FAILED, NOT_AUTHENTICATED, BAD_INPUT } = require('./StatusMessages.js');
...
exports.the_zone_update_post_put = function(req, res) {
    if (req.user) {
        try {
            let result = await dbUpdate.updatePost(req.body.postid, "content.text", req.body.message);
...
exports.the_zone_update_vote_put = async function(req, res) {
    if (req.user) {
        try {
            ...
            let result = await dbUpdate.updateVote(username, req.body.vote, req.body.postid); // may need to change from body.vote to query.vote/param.vote
