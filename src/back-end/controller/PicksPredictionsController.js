...
exports.matches_put = function (req, res) {
    res.set({
    ...
    if (req.user) {
        ...
        try {
            let match = await dbCreate.createMatch(req.body.team1, req.body.team2, req.body.start, req.body.end);
            if (match !== null) {
...
exports.matches_get = function (req, res) {
    ...
    if (req.user) {
        // user is authenticated
        try {
            let match = await dbRead.getMatch(req.query.matchid);
            res.status(200).send(match); // OK
        } catch (e) {
            console.error(e);
...
exports.matches_playoff_picks_get = function (req, res) {
    res.set({
        'Access-Control-Allow-Credentials': true,
        'Access-Control-Allow-Origin': 'http://localhost:3000'
    });
    if (req.user) {
        // user is authenticated
        try {
            let playoffPicks = await dbRead.getPlayoffPicks();
            res.status(200).send(playoffPicks); // OK
        } catch (e) {
            console.error(e);
            res.status(500).send(READ_FAILED); // Internal server error
        }
...
exports.matches_all_get = function (req, res) {
    res.set({
        'Access-Control-Allow-Credentials': true,
        'Access-Control-Allow-Origin': 'http://localhost:3000'
    });
    if (req.user) {
        // user is authenticated
        try {
            let matches = await dbRead.getMatchesAll();
            res.status(200).send(matches); // OK
        } catch (e) {
            console.error(e);
            res.status(500).send(READ_FAILED); // Internal server error
        }
...
exports.matches_update_put = function (req, res) {
    res.set({
        'Access-Control-Allow-Credentials': true,
        'Access-Control-Allow-Origin': 'http://localhost:3000'
    });
    if (req.user) {
        // user is authenticated
        try {
            let match = await dbUpdate.updateMatch(req.body.matchid, req.body.team1, req.body.team2, req.body.start, req.body.end);
            if (match) {
                res.status(200).send(match); // OK
            } else {
...
exports.matches_del = function (req, res) {
    res.set({
        'Access-Control-Allow-Credentials': true,
        'Access-Control-Allow-Origin': 'http://localhost:3000'
    });
    if (req.user) {
        // user is authenticated
        try {
            let result = await dbDelete.deleteMatch(req.body.matchid);
            if (result === 1) {
...
exports.matches_playoff_picks_all_del = function (req, res) {
    res.set({
        'Access-Control-Allow-Credentials': true,
        'Access-Control-Allow-Origin': 'http://localhost:3000'
    });
    if (req.user) {
        // user is authenticated
        try {
            let result = await dbDelete.deleteAllPlayoffPicks();
            if (result > 0) {
                res.sendStatus(200); // OK
            } else {
                res.status(404).send(NOT_FOUND); // NOT FOUND
